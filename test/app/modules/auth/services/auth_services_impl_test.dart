...
void main() {
  ...
  group('Group test register', () {
    test('Should register with success', () async {
      ...
      verify(() => repository.register(name, firebaseUserId)).called(1);
      verify(
        () => firebaseAuth.createUserWithEmailAndPassword(
          email: email,
          password: password,
    ...
    test('Should throws FirebaseAuthException', () async {
      ...
      //Assert
      expect(() => call(name, email, password), throwsA(isA<Failure>()));
      verifyNever(() => repository.register(any(), any()));
  ...
  group('Group test login', () {
    test('Should login with success', () async {
      ...
      when(
        () => mockCredentialFirebaseUser.user?.emailVerified,
      ).thenReturn(true);
      ...
      when(
        () => repository.confirmLogin(),
      ).thenAnswer((_) async => confirmLoginModel);

      when(
        () => localStorage.write(any(), any<String>()),
      ).thenAnswer((invocation) async => invocation);

      when(
        () => localSecurityStorage.write(any(), any()),
      ).thenAnswer((invocation) async => invocation);
      //Act
